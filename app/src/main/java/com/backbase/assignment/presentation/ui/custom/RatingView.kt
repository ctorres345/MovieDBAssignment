...
class RatingView @JvmOverloads constructor(
    ...
    private fun bgPaint(progress: Float) = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        ...
        color = when{
            progress < 0.5f -> context.getColor(R.color.yellow_bg_rating)
            else -> context.getColor(R.color.green_bg_rating)
        }
    ...
    private fun fillPaint(progress: Float) = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        ...
        strokeCap = Paint.Cap.ROUND
        color = when{
            progress < 0.5f -> context.getColor(R.color.yellow_fill_rating)
            else -> context.getColor(R.color.green_fill_rating)
        }
        textAlign = Paint.Align.CENTER
    }
